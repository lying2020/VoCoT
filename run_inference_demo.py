...
用法（在 VoCoT 仓库根目录执行）:
  ...
  python run_inference_demo.py --model_path luoruipu1/Volcano-7b --image figs/sample_input.jpg --query "Describe the image."

默认模型路径：优先使用 project.py 中的 volcano_7b_luoruipu1_path（若该本地目录存在），否则为 Hugging Face id luoruipu1/Volcano-7b。
默认图像：若存在 test_images/default.jpg 则优先，否则使用 figs/sample_input.jpg（见 project.images_dir）。
...
def _default_image_path() -> str:
    ...
    p_figs = os.path.join(_REPO_ROOT, "figs", "sample_input.jpg")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="输入图像；默认优先 test_images/default.jpg，否则 figs/sample_input.jpg",
    )
